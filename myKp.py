...
class KPabe(ABEnc):
    ...
    def setup(self):
        ...
        alpha = alpha1 * alpha2
        g_G1  = group.random(G1)# PK 1,2
        h_G1, h_G2 = group.random(G1), group.random(G2) # PK 3
        g2b = g_G1 ** b
        e_gg_alpha = pair(g_G1,g_G1) ** alpha

        #public parameters # 'g_G2^b':(g_G2 ** b), 'g_G2^b2':g_G2 ** (b * b),
        pk = { 'g_G1':g_G1, 'g_G2':g_G1, 'g_G2_b':g2b,
              'g_G2_b2':g2b ** b, 'h_G1_b':h_G1 ** b, 'e(gg)_alpha':e_gg_alpha }
        #secret parameters
    ...
    def keygen(self, pk, mk, policy_str):
        ...
        for x in attr_list:
            ...
            if not self.negatedAttr(x): # meaning positive
                ...
                d.append((pk['g_G1'] ** (mk['alpha2'] * shares[x])) * (pk['g_G2_b2'] ** r)) # compute D3
                d.append((pk['g_G2_b'] ** (r * group.hash(x))) * (mk['h_G1'] ** r)) # compute D4
    ...
    def encrypt(self, pk, M, attr_list):
        ...
        for i in range(len(attr_list)):
            ...
            E4[attr] = pk['g_G2_b'] ** sx[i]
            E5[attr] = (pk['g_G2_b2'] ** (sx[i] * group.hash(attr))) * (pk['h_G1_b'] ** sx[i])

        attr_list = [unicode(a) for a in attr_list]

        E1 = (pk['e(gg)_alpha'] ** s) * M
        E2 = pk['g_G2'] ** s
        return {'E1':E1, 'E2':E2, 'E3':E3, 'E4':E4 ,'E5':E5 , 'attributes':attr_list }
    ...
    def decrypt(self, E, D):
        ...
        for i in range(len(attrs)):
            ...
            if not self.negatedAttr(y):
                ...
                E_x_4 = 1
                E_x_5 = 1
                for x1 in E['attributes']:
                    E_x_4 *= E['E4'][x1] ** (1/(group.hash(y)-group.hash(x1)))
                    E_x_5 *= E['E5'][x1] ** (1/(group.hash(y)-group.hash(x1)))
                Z[y] = (pair(D[y][0],E['E2'])) / (pair(D[y][1],E_x_4) * pair(D[y][2],E_x_5))
                prodT *= Z[y] ** coeff[y]
        return E['E1'] / prodT
